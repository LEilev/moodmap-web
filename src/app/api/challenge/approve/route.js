// app/api/challenge/approve/route.js — Harmony Patch 2
...
export async function POST(req) {
  try {
    ...
    // idempotency
    if (updateId) {
      const idemKey = `idem:${pairId}:challenge:approve:${challengeId}:${updateId}`;
    ...
    if (obj.status !== 'completed') {
      return new Response(JSON.stringify({ ok:false, error: 'challenge must be completed before approval' }), { status: 400, headers: noStoreHeaders() });
    }
