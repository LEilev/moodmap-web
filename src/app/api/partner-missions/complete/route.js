...
export async function POST(req) {
  try {
    ...
    if (obj.status === 'completed' || obj.status === 'approved') {
      ...
      const curVer = Number(await redis('hget', stateKey, 'challengesVersion')) || 0;
      return new Response(JSON.stringify({ ok:true, awaitingApproval: obj.status !== 'approved', version: curVer }), { status: 200, headers: noStore() });
    }
    // Mark as completed
