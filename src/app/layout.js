...
export const metadata = {
  title: 'MoodMap',
  description: 'Understand the cycle. Survive the chaos. MoodMap helps you navigate the hormonal cycle with clarity.'
};
  ...
  return (
    <html lang="en" className="h-full scroll-smooth" suppressHydrationWarning>
      <body className="flex flex-col min-h-full bg-primary-blue text-white">
        {/* PromoteKit scripts for affiliate/Stripe links */}
        <Script 
        ...
        <Script id="promotekit-helper" strategy="afterInteractive">
          ...
          (function () {
            ...
            function setClientRefOnEmbeds(ref) {
              if (!ref) return;
              document.querySelectorAll("[pricing-table-id]").forEach(el => {
                if (!el.getAttribute("client-reference-id")) {
                  el.setAttribute("client-reference-id", ref);
                }
              });
              document.querySelectorAll("[buy-button-id]").forEach(el => {
                if (!el.getAttribute("client-reference-id")) {
                  el.setAttribute("client-reference-id", ref);
                }
              });
        ...
        {/* Locale-specific children (injected by app/[locale]/layout.js) */}
        {children}
      </body>
    </html>
  );
